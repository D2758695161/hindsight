...
function resolveSymlinks(path: string): string {
  ...
  try {
    while (!visited.has(current)) {
      visited.add(current);
      try {
        const next = realpathSync(current);
...
export function isDirectExecution(entrypoint: string | undefined = process.argv[1], moduleUrl: string = import.meta.url): boolean {
  ...
  // Resolve entrypoint through full symlink chain; moduleUrl is already the module-as-executed
  return resolveSymlinks(entrypoint) === canonicalizeExecutionPath(fileURLToPath(moduleUrl));
}
